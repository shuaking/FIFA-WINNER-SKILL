...
DATA_WEIGHT = 0.60
# Stage-dependent weight table (阶段自适应权重)
STAGE_WEIGHT_TABLE: dict[str, tuple[float, float]] = {
    "G":   (0.35, 0.65),   # Group stage: 天纪主导 (小组赛冷门多)
    "R32": (0.45, 0.55),   # Round of 32
    "R16": (0.55, 0.45),   # Round of 16
...
def is_standalone_root(root: Path) -> bool:
    return (root / "skill" / "scripts" / "worldcup_core.py").exists() and (root / "skill" / "schema").exists()


# ── wiki/ 三层结构: public | person | cache ──

def wiki_root(root: Path) -> Path:
    """Root of the wiki directory."""
    return project_root(root) / "wiki"

knowledge_base_root = wiki_root  # backward-compatible alias


def public_root(root: Path) -> Path:
    """wiki/public/ - public facts."""
    return wiki_root(root) / "public"


def person_root(root: Path) -> Path:
    """wiki/person/ - user-private data."""
    return wiki_root(root) / "person"


def cache_root(root: Path) -> Path:
    """wiki/cache/ - rebuildable cache."""
    return wiki_root(root) / "cache"


def public_edition_root(root: Path, edition: str) -> Path:
    """wiki/public/<edition>/ - public facts for an edition."""
    return public_root(root) / edition


def person_edition_root(root: Path, edition: str) -> Path:
    """wiki/person/<edition>/ - user-private data for an edition."""
    return person_root(root) / edition


def edition_data_root(root: Path, edition: str) -> Path:
    """Main edition data: wiki/public/<edition>/."""
    return public_edition_root(root, edition)
...
def public_edition_data_root(root: Path, edition: str) -> Path:
    ...
    return public_edition_root(root, edition) / "default-predictions"
...
def load_edition_data_json(root: Path, edition: str, rel_path: str | Path, default: object) -> object:
    person_path = person_edition_root(root, edition) / Path(rel_path)
    if person_path.exists():
        return load_json(person_path, default)
    public_path = public_edition_root(root, edition) / Path(rel_path)
    return load_json(public_path, default)


def raw_edition_root(root: Path, edition: str) -> Path:
    """wiki/public/<edition>/raw/ - raw source snapshots."""
    return public_edition_root(root, edition) / "raw"
...
def wiki_edition_root(root: Path, edition: str) -> Path:
    """wiki/public/<edition>/wiki/ - knowledge cards."""
    return public_edition_root(root, edition) / "wiki"


def worldcup_db_path(root: Path, edition: str) -> Path:
    """wiki/cache/<edition>/worldcup_<edition>.db."""
    return cache_root(root) / edition / f"worldcup_{edition}.db"


def bump_public_version(root: Path, edition: str, *, fixture_update: str | None = None) -> dict:
    """Update wiki/public/<edition>/version.json after public data changes."""
    import time
    ver_path = public_edition_root(root, edition) / "version.json"
    ver = load_json(
        ver_path,
        {"edition": edition, "data_hash": "sha256:initial", "updated_at": iso_now(), "schema_version": 1},
    )
    ver["data_hash"] = f"sha256:{hashlib.sha256(str(time.time()).encode()).hexdigest()[:16]}"
    ver["updated_at"] = iso_now()
    if fixture_update:
        ver["last_fixture_update"] = fixture_update
    ledger = load_match_ledger(root, edition)
    ver["match_count"] = ledger["summary"].get("match_count", len(ledger.get("matches", [])))
    teams = load_edition_data_json(root, edition, "teams.json", {"teams": []})
    ver["team_count"] = teams.get("summary", {}).get("team_count", len(teams.get("teams", [])))
    ver["schema_version"] = ver.get("schema_version", 1)
    write_json(ver_path, ver)
    return ver


public_knowledge_root = public_root  # backward-compatible alias
...
- 知识库归拢根目录：`wiki/public/{edition}/`
- 本届核心数据：`wiki/public/{edition}/` (包含 match-ledger, profiles, reports)
- 原始来源数据：`wiki/public/{edition}/raw/` (包含原始 evidence-packets)
- 知识汇总与Wiki：`wiki/public/{edition}/wiki/` (本 Wiki 目录)
...
def prediction_report_path(root: Path, edition: str, date: str) -> Path:
    """Check person/ first, then fall back to public/."""
    person_path = person_edition_root(root, edition) / "reports" / "daily-predictions" / f"{date}.json"
    return person_path if person_path.exists() else edition_data_root(root, edition) / "reports" / "daily-predictions" / f"{date}.json"
