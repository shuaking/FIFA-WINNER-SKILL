...
def project_source_root(root: Path) -> Path:
    standalone = root / "skill" / "scripts" / "worldcup_core.py"
    if standalone.exists():
        return root
    kb_project = root / PROJECT_REL
    if (kb_project / "skill" / "scripts" / "worldcup_core.py").exists():
        return kb_project
...
def export_standalone(*, root: Path, edition: str, output: Path, now: str | None = None) -> dict:
    ...
    for src, dst_rel in [
        (edition_data_src, Path("wiki") / "public" / edition),
        (raw_src, Path("wiki") / "public" / edition / "raw"),
        (wiki_src, Path("wiki") / "public" / edition / "wiki"),
    ]:
    ...
    manifest = {
        ...
        "agent_contracts": {
            ...
            "agent_card": "skill/AGENT_CARD.json",
            "tool_catalog": "skill/TOOL_CATALOG.json",
            "runbook": "skill/RUNBOOK.md",
            "guardrails": "skill/GUARDRAILS.md",
            "handoffs": "skill/HANDOFFS.md",
            "trace_events": "skill/TRACE_EVENTS.md",
        },
        "next_steps": [
            "cd exported directory",
            "read AGENT_README.md and skill/TOOL_CATALOG.json",
            "python3 -m unittest skill/tests/test_worldcup_predictor_system.py",
            "python3 skill/scripts/worldcup_edition_init.py init --edition <edition>",
        ],
        "safety_invariants": [
