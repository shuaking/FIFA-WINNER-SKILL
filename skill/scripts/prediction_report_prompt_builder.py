...
- 卦序：第{divination.get('hexagram_number', 0)}卦
- 卦辞：{divination.get('hexagram_interpretation') or divination.get('interpretation', '')}
- 时辰：{divination.get('shichen', '')}
- 命宫星曜：主队{', '.join(divination.get('home_stars', []))}；客队{', '.join(divination.get('away_stars', []))}
- 星盘解读：{divination.get('interpretation', '')}
